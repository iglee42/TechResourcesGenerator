package fr.iglee42.techresourcesgenerator.client.screen;
...
import fr.iglee42.techresourcesgenerator.client.screen.render.EnergyInfoArea;
...
public class ElectricGeneratorScreen extends AbstractContainerScreen<ElectricGeneratorMenu> {
    ...
    private void renderEnergyAreaTooltips(PoseStack pPoseStack, int pMouseX, int pMouseY, int x, int y) {
        if(isMouseAboveArea(pMouseX, pMouseY, x, y, 10, 14,16,32)) {
    ...
    private void renderProgressArrow(PoseStack pPoseStack, int x, int y) {
        if(menu.getDelay() < ConfigsForType.getConfigForType(menu.getGeneratorType()).getDelay()) {
            blit(pPoseStack, x + 105, y + 33, 176, 0, menu.getScaledProgress(), 4);
        }
    }
