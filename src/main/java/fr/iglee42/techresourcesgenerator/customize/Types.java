    ...
    private static void registerAllGenerator() {
        loadingGenerators.forEach(g->{
            switch (g.generatorType()) {
                case "basic" -> ModBlock.createManualGenerator(g.name(),g);
                case "electric" -> ModBlock.createElectricGenerator(g.name(), g);
                case "magmatic" -> ModBlock.createMagmaticGenerator(g.name(), g);
            }
            GENERATORS.add(g);
