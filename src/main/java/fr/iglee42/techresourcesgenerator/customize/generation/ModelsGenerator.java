    ...
    public static void generate() {
        ...
        Types.GENERATORS.stream().filter(g->!g.isInModBase()).forEach(generator -> {
            switch (generator.generatorType()){
                case "basic" -> blockFromParent(generator.name() + "_generator",MODID + ":block/basic_generator",  new TextureKey("1", generator.hasCustomTexture() ? generator.centerTexture() : "techresourcesgenerator:generator/"+generator.name()));
                case "magmatic" -> blockFromParent(generator.name() + "_generator",MODID + ":block/magmatic_generator",  new TextureKey("1", generator.hasCustomTexture() ? generator.centerTexture() : "techresourcesgenerator:generator/"+generator.name()));
                case "electric" -> blockFromParent(generator.name() + "_generator",MODID + ":block/electric_generator",  new TextureKey("1", generator.hasCustomTexture() ? generator.centerTexture() : "techresourcesgenerator:generator/"+generator.name()));
            }
